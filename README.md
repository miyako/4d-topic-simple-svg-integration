...
> [!NOTE]
> [`GET MOUSE`/`MOUSE POSITION`](https://developer.4d.com/docs/commands/mouse-position) can be substituted by the system variables `MOUSEX` and `MOUSEY` which return the local mouse coordinates when referenced in the context of a mouse event generated inside the boundaries of a picture object
